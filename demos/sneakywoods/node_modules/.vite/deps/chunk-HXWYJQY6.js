...
function readLength(buf, context) {
  ...
  if ((buf[context.offset] & LONG_LENGTH_MASK) === LONG_LENGTH_MASK) {
    ...
    length2 = parseInt(str, 16);
  } else {
    length2 = buf[context.offset];
    context.offset++;
  }
  return length2;
}
...
function readInteger(buf, context) {
  const length2 = readLength(buf, context);
  const start2 = context.offset;
  const end = context.offset + length2;
  const vals = [];
...
function readBitString(buf, context) {
  ...
  context.offset++;
  const bytes = buf.subarray(context.offset, context.offset + length2 - 1);
  context.offset += length2;
  if (unusedBits !== 0) {
...
function readOctetString(buf, context) {
  const length2 = readLength(buf, context);
  const bytes = buf.subarray(context.offset, context.offset + length2);
  context.offset += length2;
  return bytes;
}
...
function encodeLength(bytes) {
  ...
  const length2 = encodeNumber(bytes.byteLength);
  return new Uint8ArrayList(Uint8Array.from([
    length2.byteLength | LONG_LENGTH_MASK
  ]), length2);
}
...
var ed25519ph = ed({ domain: ed25519_domain, prehash: sha512 });
var x25519 = (() => {
  const P = ed25519_CURVE_p;
  return montgomery({
    P,
...
var ed25519_hasher = (() => createHasher2(ed25519_Point, (scalars) => map_to_curve_elligator2_edwards25519(scalars[0]), {
...
function ensureEd25519Key(key, length2) {
  key = Uint8Array.from(key ?? []);
  if (key.length !== length2) {
    throw new InvalidParametersError(`Key must be a Uint8Array of length ${length2}, got ${key.length}`);
  }
  return key;
}
...
var secp256k1 = ecdsa(Pointk1, sha2562);
...
function schnorrSign(message2, secretKey, auxRand = randomBytes(32)) {
  ...
  const m = abytes(message2, void 0, "message");
...
function hashAndSign4(key, msg, options) {
  ...
  if (isPromise(p)) {
    return p.then(({ digest }) => {
      var _a2;
      (_a2 = options == null ? void 0 : options.signal) == null ? void 0 : _a2.throwIfAborted();
      return secp256k1.sign(digest, key, {
        prehash: false,
        format: "der"
      });
    }).catch((err) => {
  ...
  try {
    return secp256k1.sign(p.digest, key, {
      prehash: false,
      format: "der"
    });
  } catch (err) {
...
function publicKeyFromRaw(buf) {
  var _a2, _b;
  if (buf.byteLength === PUBLIC_KEY_BYTE_LENGTH) {
    return unmarshalEd25519PublicKey(buf);
  } else if (buf.byteLength === PUBLIC_KEY_BYTE_LENGTH2) {
    return unmarshalSecp256k1PublicKey(buf);
  }
  const message2 = decodeDer(buf);
  const ecdsaOid = (_a2 = message2[1]) == null ? void 0 : _a2[0];
  if (ecdsaOid === ECDSA_P_256_OID || ecdsaOid === ECDSA_P_384_OID || ecdsaOid === ECDSA_P_521_OID) {
    return pkiMessageToECDSAPublicKey(message2);
  }
  if (((_b = message2[0]) == null ? void 0 : _b[0]) === RSAES_PKCS1_V1_5_OID) {
    return pkixMessageToRSAPublicKey(message2, buf);
  }
...
*/
//# sourceMappingURL=chunk-HXWYJQY6.js.map
