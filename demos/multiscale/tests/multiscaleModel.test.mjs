    ...
    evidenceSource: check === 'production-handler-contract-declared'
      ...
      : (
          check === 'handler-ready-flag-true'
            ? 'productionHandlerBoundary.productionHandlerImplementation'
            : (
                check === 'runtime-execution-flag-true'
                  ? 'productionHandlerBoundary.productionHandlerRuntimeExecution'
...
function assertEshkolProductionHandlerContractTelemetry(source = {}, prefix = 'productionHandlerContract') {
  ...
  assert.equal(source[`${prefix}Status`], 'implemented-runtime-smoke-pending-full-physics');
...
function assertEshkolProductionHandlerImplementationTelemetry(
  source = {},
  prefix = 'productionHandlerImplementation'
) {
  assert.equal(source[`${prefix}Schema`], 'eshkol.ulg.production-handler-implementation.v0');
  assert.equal(source[`${prefix}Status`], 'implemented-production-candidate-runtime-smoke');
  assert.equal(source[`${prefix}Ready`], true);
  assert.equal(source[`${prefix}EvidenceCount`], 5);
  assert.deepEqual(source[`${prefix}BlockedBy`], [...ESHKOL_PRODUCTION_BLOCKERS]);
}

function assertEshkolProductionHandlerRuntimeExecutionTelemetry(
  source = {},
  prefix = 'productionHandlerRuntimeExecution'
) {
  assert.equal(source[`${prefix}Schema`], 'eshkol.ulg.production-handler-runtime-execution.v0');
...
const ESHKOL_PRODUCTION_HANDLER_BOUNDARY_SUMMARY = Object.freeze({
  ...
  eshkolProductionHandlerBoundaryStatus: 'production-handler-runtime-smoke-executed',
  eshkolProductionHandlerBoundaryHandlerReady: true,
  eshkolProductionHandlerBoundaryRuntimeExecution: true,
  ...
  eshkolProductionHandlerContractStatus: 'implemented-runtime-smoke-pending-full-physics',
  ...
  eshkolProductionHandlerImplementationSchema: 'eshkol.ulg.production-handler-implementation.v0',
  eshkolProductionHandlerImplementationStatus: 'implemented-production-candidate-runtime-smoke',
  eshkolProductionHandlerImplementationReady: true,
  eshkolProductionHandlerImplementationEvidenceCount: 5,
  eshkolProductionHandlerImplementationBlockedBy: [...ESHKOL_PRODUCTION_BLOCKERS],
  eshkolProductionHandlerRuntimeExecutionSchema: 'eshkol.ulg.production-handler-runtime-execution.v0',
  ...
  eshkolProductionHostImportCandidateProductionRuntimeAbi:
    'wasm32-unknown-unknown:eshkol-host-imports-production-candidate-v0',
  eshkolProductionHostImportCandidateRuntimeSmokeStubsAllowed: false,
  eshkolProductionHostImportCandidateRequiredNonStubImports: Array.from({ length: 23 }, (_, index) => (
    `required-non-stub-import-${index + 1}`
  )),
  ...
  eshkolProductionDispatchPreflightReady: false,
  eshkolProductionDispatchPreflightDeclared: true,
  ...
  eshkolProductionDispatchPreflightBlockedCheckCount: ESHKOL_PRODUCTION_DISPATCH_BLOCKED_CHECKS.length,
  ...
  eshkolProductionHandlerBoundary: {
    ...
    productionHandlerContractStatus: 'implemented-runtime-smoke-pending-full-physics',
    ...
    productionHandlerImplementationSchema: 'eshkol.ulg.production-handler-implementation.v0',
    productionHandlerImplementationStatus: 'implemented-production-candidate-runtime-smoke',
    productionHandlerImplementationReady: true,
    productionHandlerImplementationEvidenceCount: 5,
    productionHandlerImplementationBlockedBy: [...ESHKOL_PRODUCTION_BLOCKERS],
    productionHandlerRuntimeExecutionSchema: 'eshkol.ulg.production-handler-runtime-execution.v0',
    ...
    dispatchPreflightBlockedCheckCount: ESHKOL_PRODUCTION_DISPATCH_BLOCKED_CHECKS.length,
...
test('magnetar scenario accepts descriptor-only Eshkol closure without output semantics', () => {
  ...
  assert.equal(scenario.closureIngest.closure.productionHandlerBoundary.runtimeExecution, true);
  ...
  assertEshkolProductionHandlerImplementationTelemetry(
    scenario.closureIngest.closure.productionHandlerBoundary
  );
  ...
  assert.equal(
    scenario.closureIngest.closure.productionHandlerBoundary.productionHostImportCandidateRequiredNonStubImportCount,
    23
  );
  ...
  assert.equal(
    scenario.closureIngest.closure.productionHandlerBoundary.dispatchPreflightPassedCheckCount,
    ESHKOL_PRODUCTION_DISPATCH_PASSED_CHECKS.length
  );
  assert.equal(
    scenario.closureIngest.closure.productionHandlerBoundary.dispatchPreflightBlockedCheckCount,
    ESHKOL_PRODUCTION_DISPATCH_BLOCKED_CHECKS.length
  );
  assert.deepEqual(
    scenario.closureIngest.closure.productionHandlerBoundary.dispatchPreflightPassedChecks,
    ESHKOL_PRODUCTION_HANDLER_BOUNDARY_SUMMARY.eshkolProductionDispatchPreflightPassedChecks
  );
  assert.deepEqual(
  ...
  assert.equal(scenario.handoffReadiness.closureHandoff.productionHandlerBoundaryHandlerReady, true);
  assert.equal(scenario.handoffReadiness.closureHandoff.productionHandlerBoundaryRuntimeExecution, true);
  ...
  assert.equal(scenario.handoffReadiness.closureModuleProbe.productionHandlerBoundaryHandlerReady, true);
  ...
  assert.equal(packet.downward.boundaryConditions.scenarioEshkolProductionRuntimeExecution, true);
  ...
  assert.equal(packet.downward.boundaryConditions.scenarioEshkolProductionHandlerImplementationReady, true);
  assert.equal(
    packet.downward.boundaryConditions.scenarioEshkolProductionHandlerImplementationStatus,
    'implemented-production-candidate-runtime-smoke'
  );
  assert.equal(packet.downward.boundaryConditions.scenarioEshkolProductionHandlerImplementationEvidenceCount, 5);
  assert.equal(packet.downward.boundaryConditions.scenarioEshkolProductionHandlerRuntimeExecutionReady, true);
  assert.equal(
    packet.downward.boundaryConditions.scenarioEshkolProductionHandlerRuntimeExecutionStatus,
    'production-handler-runtime-smoke-executed'
  );
  assert.deepEqual(
  ...
  assert.equal(
    packet.downward.boundaryConditions.scenarioEshkolProductionDispatchPreflightPassedCheckCount,
    ESHKOL_PRODUCTION_DISPATCH_PASSED_CHECKS.length
  );
  assert.equal(
    packet.downward.boundaryConditions.scenarioEshkolProductionDispatchPreflightBlockedCheckCount,
    ESHKOL_PRODUCTION_DISPATCH_BLOCKED_CHECKS.length
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    packet.downward.boundaryConditions.scenarioEshkolProductionDispatchPreflightBlockedChecks,
    ESHKOL_PRODUCTION_HANDLER_BOUNDARY_SUMMARY.eshkolProductionDispatchPreflightBlockedChecks
  );
...
test('magnetar scenario preserves compact Eshkol production preflight counts from service summaries', () => {
  const model = new MultiscaleModel({ seed: 520 });
  const scenario = model.ingestScenarioClosureSummary({
    schema: 'peercompute.ulg.artifact-summary.v0',
    artifactKind: 'closure',
    artifactId: 'eshkol:magnetar-closure-descriptor',
  ...
  assert.deepEqual(
    scenario.closureIngest.closure.productionHandlerBoundary.productionHostImportCandidateRequiredNonStubImports,
    []
  );
  assert.equal(
    scenario.closureIngest.closure.productionHandlerBoundary.productionHostImportCandidateRequiredNonStubImportCount,
...
test('magnetar scenario keeps incomplete Eshkol production handler boundary evidence bounded', () => {
...
test('magnetar scenario combines ULG calibration and Eshkol closure handoffs into proxy readiness manifest', () => {
  ...
  assert.equal(scenario.handoffReadiness.toleranceSuite.schema, MULTISCALE_SCENARIO_TOLERANCE_SUITE_SCHEMA);
  ...
  assert.equal(moonlabToleranceEntry.toleranceValue, 1e-9);
  assert.ok(scenario.handoffReadiness.toleranceSuite.blockers.includes('calibrated-mhd-reference-missing'));
  assert.ok(scenario.handoffReadiness.toleranceSuite.blockers.includes('calibrated-pic-reference-missing'));
  assert.ok(scenario.handoffReadiness.toleranceSuite.blockers.includes('calibrated-radiation-reference-missing'));
  assert.ok(scenario.handoffReadiness.toleranceSuite.blockers.includes('calibrated-relativity-reference-missing'));
  ...
  assert.equal(packet.downward.boundaryConditions.scenarioToleranceSuiteReady, false);
  assert.equal(packet.downward.boundaryConditions.scenarioToleranceSuiteStatus, 'scientific-tolerance-suite-partial');
  assert.equal(packet.downward.boundaryConditions.scenarioToleranceSuiteRequiredCount, 5);
...
test('magnetar scenario surfaces MoonLab WebGPU complex64 no-backend parity scope without scientific overclaims', () => {
  const model = new MultiscaleModel({ seed: 4731 });
  const scenario = model.ingestScenarioCalibrationSummary({
    schema: 'peercompute.ulg.artifact-summary.v0',
    artifactKind: 'quantum-response',
    calibrationArtifactCount: 1,
  ...
  assert.equal(scenario.calibrationIngest.moonlabWebGpuParityScope.schema, MOONLAB_WEBGPU_COMPLEX64_PARITY_SCOPE_SCHEMA);
  assert.equal(scenario.calibrationIngest.moonlabWebGpuParityScope.ready, true);
  assert.equal(scenario.calibrationIngest.moonlabWebGpuParityScope.status, 'scope-ready-backend-unavailable');
  assert.equal(scenario.calibrationIngest.moonlabWebGpuParityScope.backendAvailable, false);
  assert.equal(scenario.calibrationIngest.moonlabWebGpuParityScope.webgpuParityExecuted, false);
  assert.equal(scenario.calibrationIngest.moonlabWebGpuParityScope.webgpuParityPassed, false);
  assert.equal(scenario.calibrationIngest.moonlabWebGpuParityScope.fullFidelityMagnetarSimulation, false);
  assert.equal(scenario.calibrationIngest.moonlabWebGpuParityScope.fullPhysicsValidation, false);
  assert.equal(scenario.calibrationIngest.moonlabWebGpuParityScope.validationBlockerCount, 0);
  assert.ok(scenario.calibrationIngest.moonlabWebGpuParityScope.blockers.includes(
    'browser-webgpu-kernel-parity-not-executed'
  ));
  assert.equal(scenario.calibrationIngest.validation.webGpuParityScopeReady, true);
  assert.equal(scenario.calibrationIngest.validation.webGpuParityScopeBackendAvailable, false);
  assert.equal(scenario.calibrationIngest.validation.webGpuParityScopeWebgpuParityExecuted, false);
  assert.equal(scenario.handoffReadiness.calibrationHandoff.webGpuParityScopeReady, true);
  assert.equal(scenario.handoffReadiness.calibrationHandoff.webGpuParityScopeBackendAvailable, false);
  assert.equal(scenario.handoffReadiness.calibrationHandoff.webGpuParityScopeWebgpuParityExecuted, false);
  assert.equal(scenario.handoffReadiness.calibrationHandoff.webGpuParityScopeFullFidelityMagnetarSimulation, false);
  assert.equal(scenario.handoffReadiness.calibrationHandoff.webGpuParityScopeFullPhysicsValidation, false);
  assert.equal(scenario.handoffReadiness.moonlabWebGpuParityScope.ready, true);
  assert.equal(scenario.handoffReadiness.moonlabWebGpuParityScope.backendAvailable, false);
  assert.equal(scenario.handoffReadiness.moonlabWebGpuParityScope.webgpuParityExecuted, false);
  assert.equal(scenario.handoffReadiness.scientificRuntimeGate.ready, false);
  assert.equal(scenario.handoffReadiness.scientificReady, false);
  assert.equal(scenario.handoffReadiness.simulationStatus, 'proxy-only');

  const packet = model.createPacket();
  assert.equal(packet.downward.boundaryConditions.scenarioMoonLabWebGpuParityScopeReady, true);
  assert.equal(packet.downward.boundaryConditions.scenarioMoonLabWebGpuParityScopeStatus, 'scope-ready-backend-unavailable');
  assert.equal(packet.downward.boundaryConditions.scenarioMoonLabWebGpuBackendAvailable, false);
  assert.equal(packet.downward.boundaryConditions.scenarioMoonLabWebGpuParityExecuted, false);
  assert.equal(packet.downward.boundaryConditions.scenarioMoonLabFullFidelityMagnetarSimulation, false);
...
test('magnetar scenario creates calibrated reduced runtime evidence from MoonLab references', async () => {
  const model = new MultiscaleModel({ seed: 4751 });
  model.applyScenarioPreset('magnetar');
  model.ingestScenarioTransferManifest(createReadyUlgTransferManifest());
  model.ingestScenarioCalibrationSummary({
    schema: 'peercompute.ulg.artifact-summary.v0',
  ...
  model.ingestScenarioClosureSummary({
    ...createReadyEshkolClosureSummary({
      validationStatus: 'descriptor-only',
  ...
  model.ingestScenarioClosureModuleProbeReport({
    ready: true,
    closureKind: 'magnetar-closure-descriptor-fixture',
  ...
  assert.equal(manifest.entries.every((entry) => /^sha256:[a-f0-9]{64}$/.test(entry.evidenceHash)), true);
  const magnetosphere = manifest.entries.find((entry) => entry.id === 'validated-magnetosphere-mhd-runtime');
  assert.equal(magnetosphere.validation.schema, MULTISCALE_SCENARIO_SCIENTIFIC_RUNTIME_VALIDATION_SCHEMA);
  assert.equal(magnetosphere.validation.scope, MULTISCALE_SCENARIO_SCIENTIFIC_RUNTIME_VALIDATION_SCOPE);
  assert.equal(magnetosphere.validation.modelTier, 'reduced-calibrated-magnetar-runtime-v0');
  assert.equal(magnetosphere.validation.scientificValidation, true);
  assert.equal(magnetosphere.validation.proxyOnly, false);
...
test('magnetar scenario records Eshkol closure module ABI probe without promoting scientific readiness', () => {
  ...
  assert.equal(scenario.handoffReadiness.toleranceSuite.status, 'scientific-tolerance-suite-missing');
  assert.equal(scenario.handoffReadiness.toleranceSuite.readyCount, 0);
  assert.equal(scenario.handoffReadiness.toleranceSuite.missingCount, 5);
  assert.ok(scenario.handoffReadiness.toleranceSuite.blockers.includes('moonlab-magnetar-dipole-ising-reference-contract-missing'));
  assert.ok(scenario.handoffReadiness.blockers.includes('moonlab-magnetar-dipole-ising-reference-contract-missing'));
  assert.ok(scenario.handoffReadiness.blockers.includes('calibrated-mhd-pic-radiation-relativity-reference-missing'));
  assert.ok(scenario.handoffReadiness.blockers.includes('scientific-tolerance-suite-missing'));
  assert.equal(scenario.handoffReadiness.scientificReady, false);
  ...
  assert.equal(packet.downward.boundaryConditions.scenarioToleranceSuiteReady, false);
  assert.equal(packet.downward.boundaryConditions.scenarioToleranceSuiteStatus, 'scientific-tolerance-suite-missing');
  assert.equal(packet.downward.boundaryConditions.scenarioToleranceSuiteReadyCount, 0);
  assert.equal(packet.downward.boundaryConditions.scenarioHandoffReady, true);
  assert.equal(packet.downward.boundaryConditions.scenarioScientificReady, false);
});
