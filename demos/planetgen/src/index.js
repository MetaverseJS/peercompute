...
async function generateWorld(presetKey) {
    ...
    try {
        ...
        rebuildWaterCycleClouds(sunDir, { force: true });
        rebuildMoon();
        applyPlanetScale();
...
function rebuildWaterCycleClouds(sunDir, options = {}) {
    if (!lastPlanetSettings) return;
    ...
    const signature = [
        ...
        volMeta.atmoThicknessM ?? 0
    ].join('|');

    if (!force && waterCycleCloud && waterCycleCloud.userData?.signature === signature) {
        const u = waterCycleCloud.userData?.uniforms || {};
        if (u.color?.value) u.color.value.set(settings.color);
        if (u.opacity) u.opacity.value = settings.alpha;
        if (u.quantity) u.quantity.value = settings.quantity;
        if (u.noiseScale) u.noiseScale.value = Math.max(1, settings.resolution);
        if (u.mode) {
            u.mode.value = settings.mode === 'billow' ? 1 : settings.mode === 'cellular' ? 2 : 0;
        }
        waterCycleCloud.userData.settings = settings;
        return;
    }
...
function animate() {
    ...
    if (planetManager?.setOceanWindVector) {
        planetManager.setOceanWindVector(windVec);
    }
...
function getWeatherSimMode() {
    ...
    if (v === '2d') return '2d';
    if (v === 'holistic') return 'holistic';
    return '3d';
}

function isWeatherMode3d(mode) {
    return mode !== '2d';
}
    ...
    try {
        if (isWeatherMode3d(mode)) {
            ...
            if (!waterCycleSystem3D) {
                ...
                waterCycleSystem3D = await WaterCycleVolumeSystem.create({
                    volumeN: desiredN,
                    surfaceW: 256,
                    surfaceH: 128,
            ...
            // TODO: swap in the holistic solver when the new mode is implemented.
            waterCycleSystem = waterCycleSystem3D;
        } else {
            if (!waterCycleSystem2D) {
                const device = await getSharedDevice();
                waterCycleSystem2D = await WaterCycleSystem.create({
                    gridWidth: 256,
                    gridHeight: 128,
                    device: device || undefined
                });
...
function updateWeatherAutoScale(deltaSeconds) {
    ...
    if (weatherAutoState.overBudgetS >= WEATHER_AUTO_LIMITS.downDelayS) {
        if (isWeatherMode3d(mode) && weatherAutoState.tier3d > 0) {
        ...
        if (isWeatherMode3d(mode) && weatherAutoState.tier3d < WEATHER_AUTO_TIERS_3D.length - 1) {
            weatherAutoState.tier3d += 1;
            applyWeatherAutoTier(mode);
