...
export function decodeWindFieldFromAuxTexture(auxTexture, gridW, gridH, options = {}) {
    ...
    if (w === gridW && h === gridH) {
        for (let i = 0; i < cellCount; i++) {
            const idx = i * 4;
            field[i * 2] = decodeWind(data[idx + 2] ?? 0, maxWind);
            field[i * 2 + 1] = decodeWind(data[idx + 3] ?? 0, maxWind);
        }
        return field;
    }
    ...
    for (let y = 0; y < gridH; y++) {
        ...
        for (let x = 0; x < gridW; x++) {
            ...
            const i = (y * gridW + x) * 2;
            field[i] = decodeWind(px.b ?? 0, maxWind);
            field[i + 1] = decodeWind(px.a ?? 0, maxWind);
        }
    }
