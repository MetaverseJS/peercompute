...
- Multiplayer runtime hardening: NetViz relay-line rendering now requires active direct relay transport evidence, the runtime P2P harness now covers NetViz attach plus filterable multiplayer demo subsets, CubeChat renegotiation/remote-stream races were reduced, and DaddyGo now exposes deterministic `?e2e=1` hooks for headless score-sync validation.
