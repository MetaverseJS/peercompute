...
- 2026-04-04: committed the latest multiplayer/runtime fixes as `d63abacb` (`Harden multiplayer runtime gates and browser startup`), pushed `origin/demo-fixes`, then rechecked GitHub Pages after the publish delay. By `2026-04-04 01:35 UTC`, the live `/peercompute/netviz/` and `/peercompute/hyperborea/cb.html` pages had switched to the new asset hashes (`index-BIMwYj1U.js`, `cb-BLxBAQ0K.js`), confirming the latest NetViz and Hyperborea frontend bundles are now live.
- 2026-04-04: ran live host Chromium probes against `https://metaversejs.github.io/peercompute/cubechat/` in isolated private rooms. Production CubeChat media passes at 3 peers (every page reached `peerCount=2`, `remoteStreamCount=2`, `remoteVideoReadyCount=2`) but fails at 4 peers, where the 120 s gate timed out with partial convergence (`remoteStreamCount` ending at `2/1/2/0` across the four pages and only one page rendering even a single fully ready remote video). This is now the concrete prod multiplayer regression to chase, separate from the local `npm run test:runtime:p2p` pass.
- 2026-04-04: replaced the chaos-lab probe's ad hoc simulated key taps with a reusable player behavior harness in `net-chaos-lab/agent/player-behavior-harness.mjs`. CubeChat now uses a Quake-style arena bot that reads live remote peer positions from `window.__cubechat`, patrols when alone, and switches to pursue/orbit/evade/jump behavior when other players appear; the other demo profiles now use repeatable skirmish phase plans instead of one-shot key pulses. Unit coverage, a live single-page CubeChat smoke through `agent/probe.mjs`, and a live two-page CubeChat smoke that showed both bots acquiring each other in `mode:pursue` all pass.
- 2026-04-04: generalized the bot harness behind a shared browser contract in `demos/shared/peercomputeBotBridge.js`. The chaos-lab harness now prefers registered `window.__PEERCOMPUTE_BOT_BRIDGES__` adapters over game-specific probe hacks, and both CubeChat plus Hyperborea now register `snapshot/applyAction/clearAction` bridges so the same arena bot can be dropped into multiple games with minimal adapter code.
- 2026-04-04: split the shared arena logic into a modular Quake III-style bot core under `net-chaos-lab/agent/quake3/` (`math.mjs`, `world-model.mjs`, `bot-core.mjs`) so steering/goal/combat logic is reusable independently of the probe harness. SneakyWoods now registers the shared bot bridge, Hyperborea and SneakyWoods map generic bot `primary` actions into real attacks, and local built-doc Chromium proofs now show both SneakyWoods plus Hyperborea running through `driver: "peercompute-bot-bridge"` with `behavior: "quake3-bot-core"` and real movement in the page state.
- 2026-04-04: continued the Quake-style AI split by adding `personalities.mjs`, `memory.mjs`, and `navigation.mjs` under `net-chaos-lab/agent/quake3/`. The bot stack now supports named archetypes, recent-goal/nav penalties, nav-point route selection, hazard-aware steering, and metadata-driven personality overrides. The updated core passes the expanded behavior suite and a fresh local SneakyWoods built-doc Chromium smoke still reports `driver: "peercompute-bot-bridge"` / `behavior: "quake3-bot-core"` with real player movement.
- 2026-04-04: refreshed the root `README.md` so it now inventories the real shipped PeerCompute functionality: browser transport/topology behavior, layered state + compute runtimes, NetViz/chaos-lab observability, shared bot infrastructure, demo coverage, test gates, and backend/deploy workflows.
- 2026-04-04: traced the missing Universes cosmic web to the production Vite build path, not the generator itself. In built/docs/GitHub Pages, `ComputeManager` workers were emitted as `data:` modules whose internal relative imports broke, so `generateUniverseDensity` / `generateUniverseData` hung before fallback and the universe view rendered black. `ComputeManager` now detects worker failure, removes the broken worker, and re-executes stranded tasks inline; fresh built-doc Chromium checks show the cosmic web visible again.
...
- Done (2026-04-04): added shared in-browser bot hosting through `demos/shared/peercomputeBots.js` and wired CubeChat, Hyperborea, plus SneakyWoods settings screens to expose `Bots -> Add Count / Archetype / Add Bots / Clear Bots`. Bot launches now carry room/password state, start the Quake-style local runtime automatically through each demo's shared bridge, and were verified in host-run headless Chromium from the settings UI with `ready/running` bot status on all three demos. CubeChat also now skips media capture for bot launches and uses non-fatal relay-listen startup in both the main room node and room-directory node so local/docs bot sessions still boot under the headless/local HTTP harness.
- Done (2026-04-04): deployed the bot-settings/bot-host work to `origin/demo-fixes` as commit `c7b39cb8` (`Add shared in-browser demo bots`). GitHub Pages later switched the live public bundles to `cubechat/assets/index-D0l_3sgy.js`, `hyperborea/assets/cb-B6gVXbeJ.js`, and `sneakywoods/assets/index-CNBDqZqF.js`, confirming the new settings-screen bot controls are in the published site.
- Done (2026-04-04): deployed the Universes cosmic-web recovery as commit `a673a734` (`Fix Universes built cosmic web fallback`). GitHub Pages switched the live Universes bundle from `assets/index-DJnP2HYK.js` to `assets/index-CIrBa6KX.js`, and a fresh headless Chromium check against `https://metaversejs.github.io/peercompute/universes/` showed the cosmic web visible again on the published page.
